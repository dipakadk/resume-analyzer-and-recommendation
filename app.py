...
#pafy.set_api_key("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36")
import nltk
...
import pymysql # used for connecting the database
from courses import data_course,web_course,android_course,ios_course,uiux_course,resume_videos,interview_videos
import pafy
# Set the user-agent to mimic a web browser request
pafy.set_api_key("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36")
...
def run():
    ...
    st.image(img)
    st.title('Resume Analyzer and Recommendation System')
    st.sidebar.markdown('Choose User')
    activities=['user','admin']
    choice=st.sidebar.selectbox('Choose the option',activities)
    ...
    if choice == 'user':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                res_vid_title = youtube_video(resume_vid)
                ...
                int_vid_title = youtube_video(interview_vid)
        ...
        if st.button('Login'):
            if ad_user == 'admin' and ad_password == 'texas':
